...
object StickResolver_Impl extends StickResolver {
  ...
  private def iterate(
      open: Seq[BlockCoord],
      closed: Set[BlockCoord] = Set.empty
  ): Set[BlockCoord] = open match {
    case Seq() => closed
          ...
          for (s <- 0 until 6) {
            if (StickRegistry.resolveStick(world, next, s)) {
              ...
              if (!excl(to) && !closed(to) && !open.contains(to))
                if (
                  !world.isAirBlock(to.x, to.y, to.z) && !RelocationAPI.instance
                    .isMoving(world, to.x, to.y, to.z)
