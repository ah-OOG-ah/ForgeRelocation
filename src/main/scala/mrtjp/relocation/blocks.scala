...
class BlockMovingRow
    extends InstancedBlock("relocation.blockmovingrow", Material.iron) {
  setHardness(-1f)
...
class TileMovingRow extends InstancedBlockTile {
  ...
  def pushEntities(r: BlockRow, progress: Double) {
    val box = Cuboid6.full.copy
      .add(new Vector3(r.preMoveBlocks.head))
    ...
    world.getEntitiesWithinAABBExcludingEntity(null, box) match {
      case list: JList[_] =>
        for (e <- list.asInstanceOf[JList[Entity]])
          e.moveEntity(d.x, d.y max 0, d.z)
      case _ =>
    }
