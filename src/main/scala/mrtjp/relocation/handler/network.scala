...
object RelocationSPH extends RelocationPH with IServerPacketHandler {
  ...
  private val updateMap =
    MMap[World, MMap[Set[ChunkCoordIntPair], MCByteStream]]()
  private val chunkWatchers = new MHashMap[Int, MSet[ChunkCoordIntPair]]
    with MMultiMap[Int, ChunkCoordIntPair]
  ...
  private def getDescPacket(
      world: World,
      chunks: Set[ChunkCoordIntPair]
  ): PacketCustom = {
