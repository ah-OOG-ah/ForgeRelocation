...
object Utils {
  def rescheduleTicks(
      ...
      allBlocks: Set[BlockCoord],
      dir: Int
  ) = world match {
    case world: WorldServer =>
      val hash = ObfuscationReflectionHelper
        .getPrivateValue(
          classOf[WorldServer],
          world,
          "field_73064_N",
      ...
      val tree = ObfuscationReflectionHelper
        .getPrivateValue(
          classOf[WorldServer],
          world,
          "field_73065_O",
      ...
      val list = ObfuscationReflectionHelper
        ...
        .asInstanceOf[JAList[NextTickListEntry]]

      val isOptifine = world.getClass.getName == "WorldServerOF"

      val chunks = allBlocks
        .map(b => world.getChunkFromBlockCoords(b.x, b.z))
        .filter(_ != null)

      val scheduledTicks = chunks.flatMap(ch =>
        world
          .getPendingBlockUpdates(ch, !isOptifine)
